class Table {
  ...
  constructor(num: number) {
    ...
    if (num == 2) document.getElementById("con").appendChild(div);
    else document.getElementById("con").prepend(div);
  }
}
let current1: string;
let count1 = 0;
...
class TeamPlay {
    ...
    return () => {
      ...
      if (this.ctime.time > 0) {
        ...
        let td = document.getElementById(
          team.toString() + this.i.toString() + this.j.toString()
        );
        if (td != null) {
          ...
          if (this.j == 6 || run == 0) {
            document.getElementById(
              team.toString() + this.i.toString() + 6
            ).innerText = this.playerTotal.toString();
            this.playerTotal = 0;
            this.j = 0;
            this.i += 1;
          }
  ...
  timeout(): () => void {
    return () => {
      ...
      if (this.playerTotal != 0) {
        document.getElementById(this.t + this.i.toString() + 6).innerText =
          this.playerTotal.toString();
        this.playerTotal = 0;
      }
